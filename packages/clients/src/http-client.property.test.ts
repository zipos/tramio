...
const externalUrlArb = fc
  .record({
    ...
    path: fc.stringOf(fc.constantFrom(...'/abcdefghijklmnopqrstuvwxyz0123456789-_'.split('')), {
      minLength: 1,
      maxLength: 30,
    }),
  })
...
const loopbackUrlArb = fc
  .record({
    ...
    host: fc.oneof(
      ...
      fc.constant('[::1]'),
      fc.constant('0.0.0.0'),
      // Only the specific emulator gateway IPs are loopback (FIX 2).
      fc.constant('10.0.2.2'),
      fc.constant('10.0.3.2'),
    ),
...
describe('Property 15: No cellular network calls during an active tour', () => {
  ...
  it('always exempts loopback/IPC addresses regardless of tour state', () => {
    fc.assert(
      fc.asyncProperty(loopbackUrlArb, fc.boolean(), async (url, tourActive) => {
        const client = createHttpClient({
